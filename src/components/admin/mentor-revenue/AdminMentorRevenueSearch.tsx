...
interface AdminMentorRevenueSearchProps {
  searchQuery: string;
  ...
  hasRevenue: string;
  onHasRevenueChange: (value: string) => void;
...
const HAS_REVENUE_OPTIONS = [
  ...
  { value: 'true', label: 'With revenue' },
  { value: 'false', label: 'No revenue' },
];

export function AdminMentorRevenueSearch({
  searchQuery,
  onSearchChange,
  fromDate,
  onFromDateChange,
  ...
  onHasRevenueChange,
  tiers,
}: AdminMentorRevenueSearchProps) {
  const hasActiveFilters = !!fromDate || !!toDate || !!status || !!tierId || !!verified || !!hasRevenue;
    ...
    <AdminSearchPanel>
      <div className="flex flex-col gap-3 xl:flex-row xl:items-end">
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 xl:flex xl:shrink-0 xl:flex-wrap xl:items-end xl:gap-2">
          ...
          <div className="flex min-w-0 flex-col gap-1 xl:w-36">
            <label className="text-xs font-medium text-slate-500">Revenue</label>
            <select
              value={hasRevenue}
              onChange={(e) => onHasRevenueChange(e.target.value)}
              className={cn(adminSelectClass, 'min-w-0 w-full')}
            >
              {HAS_REVENUE_OPTIONS.map((opt) => (
                <option key={opt.value || 'all'} value={opt.value}>
                  {opt.label}
                </option>
              ))}
      ...
      {hasActiveFilters ? (
        <div className="flex flex-wrap gap-2">
          ...
          {hasRevenue ? (
            <span className={adminFilterPillClass}>
              {HAS_REVENUE_OPTIONS.find((opt) => opt.value === hasRevenue)?.label ?? hasRevenue}
              <button type="button" onClick={() => onHasRevenueChange('')} className="hover:text-primary/70">
                <X className="h-3 w-3" />
              </button>
            </span>
          ) : null}
