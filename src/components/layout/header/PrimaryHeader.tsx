...
export default function PrimaryHeader() {
  ...
  const navLinkClass = (path: string) =>
    cn(
      ...
      isActive(path) ? 'bg-primary/8 text-primary' : 'text-gray-600 hover:bg-gray-50 hover:text-primary'
    );
                ...
                {isUserMenuOpen && (
                  <div className="absolute right-0 z-50 mt-2 w-60 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-lg">
                    <div className="border-b border-gray-100 bg-gray-50/70 px-4 py-3">
                      <p className="truncate text-sm font-medium text-gray-900">{user?.fullName || 'My Account'}</p>
                      {user?.email ? (
